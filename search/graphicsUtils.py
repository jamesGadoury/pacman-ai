...
def begin_graphics(
    width=640 * 2, height=480 * 2, color=formatColor(0, 0, 0), title=None
):
...
def remove_from_screen(x, d_o_e=None, d_w=tkinter._tkinter.DONT_WAIT):
    d_o_e = _root_window.dooneevent if d_o_e is None else d_o_e
    _canvas.delete(x)
    d_o_e(d_w)
...
def move_to(object, x, y=None, d_o_e=None, d_w=tkinter._tkinter.DONT_WAIT):
...
def move_by(object, x, y=None, d_o_e=None, d_w=tkinter._tkinter.DONT_WAIT, lift=False):
    if y is None:
        try:
            x, y = x
        except:
